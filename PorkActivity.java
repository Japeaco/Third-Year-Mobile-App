...
public class PorkActivity extends AppCompatActivity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        porkList = new ArrayList<>();
        adapter = new FoodListAdapter(this, R.layout.food_items, porkList);
        gridView.setAdapter(adapter);

        Cursor cursor = MainActivity.mydb.getData("SELECT * FROM RECIPES WHERE category = 'Pork'");
        porkList.clear();
        while (cursor.moveToNext()){
            ...
            porkList.add(new Food(name, image, ing1, ing2, ing3, ing4, ing5, category, meal, fav));
        }
        adapter.notifyDataSetChanged();

        gridView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            public void onItemClick(AdapterView<?> parent, View v, int position, long id) {
                String repName = porkList.get(position).getName().toString().replaceAll("\\s+","");
                String recipeName = "com.example.admin.easyeats." + repName + "Activity";
                try {
                    Class<?> c = Class.forName(recipeName);
                    startActivity(new Intent(PorkActivity.this, c));
                } catch (ClassNotFoundException e) {
                    Toast.makeText(PorkActivity.this, e.toString(),
