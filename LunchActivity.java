...
public class LunchActivity extends AppCompatActivity
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        adapter = new FoodListAdapter(this, R.layout.food_items, lunchList);
        gridView.setAdapter(adapter);

        Cursor cursor = MainActivity.mydb.getData("SELECT * FROM RECIPES WHERE meal = 'Lunch'");
        lunchList.clear();
        while (cursor.moveToNext()){
            ...
            lunchList.add(new Food(name, image, ing1, ing2, ing3, ing4, ing5, category, meal, fav));
        }
        adapter.notifyDataSetChanged();

        gridView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            public void onItemClick(AdapterView<?> parent, View v, int position, long id) {
                String repName = lunchList.get(position).getName().toString().replaceAll("\\s+","");
                String recipeName = "com.example.admin.easyeats." + repName + "Activity";
                try {
                    Class<?> c = Class.forName(recipeName);
                    startActivity(new Intent(LunchActivity.this, c));
                } catch (ClassNotFoundException e) {
                    Toast.makeText(LunchActivity.this, e.toString(),
    ...
    public boolean onNavigationItemSelected(MenuItem item) {
        ...
        if (id == R.id.nav_home) {
            ...
            Intent myIntent = new Intent(LunchActivity.this,
                    MainActivity.class);
